...
if (isset($_POST['submit'])) {
    ...
    $sql_cliente = "INSERT INTO adm(nome_completo, cpf, endereco, telefone, loja_id) 
        VALUES ('$nome', '$cpf', '$Endereco', '$telefone', '$loja_id')";
...
<body>
    <form class="login" action="" method="post">
        ...
        <select name="loja_id">
            ...
            <?php
            // Aqui você irá gerar as opções dinamicamente no PHP
            // Recupere os dados da tabela "loja"
            $sqlLoja = "SELECT id, rua FROM loja";
            mysqli_set_charset($conexao, "utf8");
            $resultLoja = mysqli_query($conexao, $sqlLoja);
            

            // Use um loop para gerar as opções
            while ($rowLoja = mysqli_fetch_assoc($resultLoja)) {
                $idLoja = $rowLoja['id'];
                $ruaLoja = $rowLoja['rua'];
                echo "<option value='$idLoja'>$ruaLoja</option>";
            }
            ?>
        </select>
        <label>Loja</label>
