...
if(isset($_POST['submit']))
    {
        ...
        $sql_adm = "UPDATE adm SET nome_completo = '$nome', cpf = '$cpf', endereco = '$endereco', telefone = '$telefone' WHERE id = $id";


        $result_adm = mysqli_query($conexao, $sql_adm);

        if ($result_adm) {
            // Obtém o ID do adm recém-cadastrado
            $id_adm = mysqli_insert_id($conexao);
    
            // Insere o novo login associado ao ID do adm
            $sql_login = "UPDATE login SET email = '$email', senha = '$senha' WHERE id_adm = $id";
